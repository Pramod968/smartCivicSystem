...
def submit_complaint(request):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        _send_submission_email(complaint)

        messages.success(request,
            f'✅ Complaint submitted! Your ID is <strong>{complaint.complaint_id}</strong>. A confirmation email has been sent.',
            extra_tags='safe')
...
def admin_complaint_detail(request, pk):
    ...
    if request.method == 'POST' and form.is_valid():
        ...
        updated.save()
        messages.success(request, '✅ Complaint updated successfully. Email notification sent to citizen.')
...
def _send_submission_email(complaint):
    """Send receipt notification email to the citizen."""
    subject = f'[SmartCivic] Complaint Received — {complaint.complaint_id}'
    message = (
        f'Hi {complaint.user.first_name or complaint.user.username},\n\n'
        f'We have received your report: "{complaint.title}".\n'
        f'Your unique Complaint ID is: {complaint.complaint_id}\n\n'
        f'You can track its progress here: '
        f'http://localhost:8000/complaints/track/{complaint.complaint_id}/\n\n'
        f'Thank you for helping us improve our city!\n'
        f'— SmartCivic Team'
    )
    try:
        send_mail(
